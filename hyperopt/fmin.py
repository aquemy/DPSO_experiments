...
class FMinIter(object):
    ...
    def run(self, N, block_until_done=True):
        ...
        def __timeout(start, max_time):
            return time.time() > start + max_time if max_time is not None else False
        ...
        with std_out_err_redirect_tqdm() as orig_stdout:
            with tqdm(total=N+qlen, file=orig_stdout, postfix='best loss: ?',
                ...
                while n_queued < N:
                    qlen = get_queue_len()
                    timeout = __timeout(start, self.max_time)
                    while (qlen < self.max_queue_len and n_queued < N) and not timeout:
                    ...
                    if self.asynchronous:
                        ...
                        self.serial_evaluate()
                    try:
                        res = [d['result']['loss'] for d in
                                         self.trials.trials if
                                         d['result']['status'] == 'ok']
                        if not len(res) and timeout:
                            stopped = True
                        best_loss = min(res)
                    ...
                    logger.info('Timeout occured. Exit.')
                    if timeout:
                        n_queued = N
...
def fmin(fn, space, algo, max_evals, max_time=None, trials=None, rstate=None,
    ...
    if max_evals is None and max_time is None:
        raise Exception("max_evals or max_time must be specified")
